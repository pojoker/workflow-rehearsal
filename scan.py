...
def invariants():
    ...
    # ⑥白名单
    # 须与 .githooks/pre-commit 的 WL 逐字一致(两处重复定义,改一处必改另一处)
    # '.git': worktree 下 .git 是文件不是目录,不列入则误报越位(远程代理绕闸根因)
    WL={'README.md','CLAUDE.md','tree.yaml','points.csv','edges.csv','triage.csv','words.txt','scan.py','render.py','participation.py','make_participation_pdf.py','RESTART-v2.md','.gitignore','.git','.DS_Store'}
